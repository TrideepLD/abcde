...
public class Algo {
	...
	public Server bestFit(Job job) {
		...
		for (Server serv : servers) {
			if ((serv.coreCount >= job.cpuCores && serv.memory >= job.memory && serv.disk >= job.disk)) {
		...
		if (found) {
			...
			servAlt.id = -1; // If this isn't zero, server thinks it doesn't exist.
			return servAlt;
		}
	}
	...
	public Server sjf(Job job) {
		...
		// for (Server serv : servers) {
		// 	if ((serv.coreCount >= job.cpuCores && serv.memory >= job.memory && serv.disk >= job.disk && serv.availableTime < minAvail && serv.state != 4)) {
		// 		// The fitness value of a job to a server is defined as the difference between the number of cores the job requires and that in the server.
		// 			minAvail = serv.availableTime;
		// 			found = true;
		// 			newServer = serv;
